...
protected:
	...
	void md_connie_3in1_map(address_map &map) ATTR_COLD;
...
INPUT_PORTS_START( connie3in1 )
	...
	PORT_START("COINS")
	PORT_BIT( 0x0001, IP_ACTIVE_HIGH, IPT_COIN1 ) PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(md_connie_3in1_state::coin_inserted), 0) PORT_IMPULSE(1)
...
void md_connie_3in1_state::connie3in1(machine_config &config)
{
	...
	m_maincpu->set_addrmap(AS_PROGRAM, &md_connie_3in1_state::md_connie_3in1_map);
}

void md_connie_3in1_state::machine_start()
{
	md_boot_state::machine_start();
	m_gamebank->configure_entries(0, 4, memregion("maincpu")->base(), 0x100000);
}

void md_connie_3in1_state::machine_reset()
{
